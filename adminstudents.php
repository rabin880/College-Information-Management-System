...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['search'])) {
    ...
    $sql = "SELECT s.sid, s.name, s.username, s.password, s.batch, s.faculty, s.address, s.gender, s.email, s.dob, s.phoneno, s.parentsname, s.parentscontact, s.photo, c.classname, b.batch_year, f.name as faculty_name 
        FROM studentlog s
        LEFT JOIN class c ON s.classid = c.cid
        LEFT JOIN faculty f ON s.faculty = f.fcid
    ...
    $sql = "SELECT s.sid, s.name, s.username, s.password, s.batch, s.faculty, s.address, s.gender, s.email, s.dob, s.phoneno, s.parentsname, s.parentscontact, s.photo, c.classname, b.batch_year, f.name as faculty_name
            ...
            LEFT JOIN faculty f ON s.faculty = f.fcid
            LEFT JOIN batches b ON s.batch = b.batch_id
            WHERE s.name LIKE ? OR s.username LIKE ? OR b.batch_year LIKE ? OR f.name LIKE ?";  // Missing semicolon here!

    $stmt = $conn->prepare($sql);
    $likeQuery = "%$searchQuery%";
    $stmt->bind_param("ssss", $likeQuery, $likeQuery, $likeQuery, $likeQuery); // Bind the four parameters
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    <div class="col-md-6"><strong>Faculty:</strong> <span id="viewFaculty"></span></div>
                </div>
                <div class="row mb-3">
    ...
    <div class="modal-dialog modal-lg">
        ...
        <form id="studentForm" method="POST" enctype="multipart/form-data"> 
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <label for="dob" class="form-label">Date of Birth  (MM/DD/YYYY)
...
$(document).on("click", ".view-button", function () {
    ...
    $("#viewFaculty").text(student.faculty_name);
