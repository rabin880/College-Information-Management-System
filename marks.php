...
// Fetch subjects for a given exam
if (isset($_GET['fetch']) && $_GET['fetch'] === 'subjects') {
    $examId = $_GET['exam_id'];
    $query = "SELECT subject.subid, subject.subname, subject.fullmarks FROM subject 
...
if (isset($_GET['fetch']) && $_GET['fetch'] === 'existing_marks') {
    ...
    // Check if examId and classId are set and valid
    if (empty($examId) || empty($classId)) {
        die('Exam ID or Class ID is missing.');
    }
    ...
    $query = "SELECT marks.student_id, marks.subject_id, marks.mark
              ...
              WHERE marks.exam_id = $examId AND studentlog.classid = $classId";

    // Log the query with parameters
    error_log("Executing query: $query with examId=$examId and classId=$classId");
    ...
    if (empty($result->num_rows < 0)) {
        echo json_encode(['message' => 'No marks found for the given exam and class.',$classId,$examId]);
    } else {
    ...
    <style>
        * {
        ...
        body {
            ...
            padding-top: 60px;
            /* Prevent the content from being hidden under the fixed header */
        }
        ...
        .sidebar {
            ...
            position: fixed;
            top: 60px;
            /* Aligns under the fixed header */
            bottom: 0;
            left: 0;
            /* Ensures it is aligned to the left */
            overflow-y: auto;
        ...
        .sidebar ul li a {
            ...
            font-size: 15px;
            padding: 12px 8px;
            /* Adjust the padding (vertical horizontal) between menu items */
            display: block;
            /* Make the link fill the entire area, improving clickability */
            border-radius: 4px;
            /* Optional: Adds rounded corners to the menu items */
        ...
        .main-content {
            margin-left: 160px;
            /* Set the same margin as sidebar width */
            padding: 20px;
            ...
            overflow-y: auto;
            /* Scrollbars on vertical content */
            scrollbar-width: none;
            /* Hide scrollbar in Firefox */
            overflow-x: auto;
            /* Allow horizontal scrolling if needed */
            z-index: 1;
            /* Ensure content appears above the sidebar */
            position: relative;
        }
    ...
    <div class="d-flex">
        ...
        <nav class="sidebar bg-dark text-white p-3" style="width: 150px;">
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    <a class="nav-link text-white" data-bs-toggle="collapse" href="#manageSettings" role="button"
                        aria-expanded="false" aria-controls="manageSettings">
                        <i class="fas fa-cogs me-1"></i> Settings
                    </a>
                    <div class="collapse" id="manageSettings">
                        <ul class="nav flex-column ms-3">
                            <li class="nav-item">
                                <a href="basicSettings.php" class="nav-link text-white d-flex align-items-center"
                ...
                <!-- Collapsible Manage Settings -->
                <li class="nav-item">
                    <a class="nav-link text-white" data-bs-toggle="collapse" href="#manageclass" role="button"
                        aria-expanded="false" aria-controls="manageclass">
                        <i class="fas fa-cogs me-1"></i> Class
                    </a>
                    <div class="collapse" id="manageclass">
                        <ul class="nav flex-column ms-3">
                            <li class="nav-item">
                                <a href="adminclass.php" class="nav-link text-white d-flex align-items-center"
                                    style="white-space: nowrap;">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="faculty" name="faculty">
                                    ...
                                    <?php
                                    $query = "SELECT fcid, name FROM faculty";
                                    $result = $conn->query($query);
                                    while ($row = $result->fetch_assoc()) {
            ...
            Promise.all([
                ...
                students.forEach(student => {
                    ...
                    subjects.forEach(subject => {
                        ...
                        // Debugging: Log the existing marks object and keys
                        

                        // Pre-populate existing marks
                        if (
                            existingMarks[student.student_id] &&
                            existingMarks[student.student_id][subject.subject_id] // Align with 'subject_id' from the database
                        ) {
                            console.log(existingMarks[student.student_id][subject.subject_id])
                            input.value = existingMarks[student.student_id][subject.subject_id];
                        }
