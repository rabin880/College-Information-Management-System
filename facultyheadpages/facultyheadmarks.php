...
if (isset($_GET['fetch']) && $_GET['fetch'] === 'existing_marks') {
    ...
    // Query to fetch marks, ensuring classId is present in classid (which is now a comma-separated string)
    $query = "SELECT marks.student_id, marks.subject_id, marks.mark
              FROM marks
              JOIN studentlog ON marks.student_id = studentlog.sid
              WHERE marks.exam_id = ? AND FIND_IN_SET(?, studentlog.classid) > 0";

    $stmt = $conn->prepare($query);
    $stmt->bind_param("ii", $examId, $classId);  // Ensure proper binding
    $stmt->execute();
    $result = $stmt->get_result();
