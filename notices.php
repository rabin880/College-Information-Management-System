...
<section id="recent-notices" class="container mt-5">
    <div class="d-flex flex-column align-items-center text-center">
        <h1 class="section-title mt-5 pb-5">All Notices</h1>
    </div>

    <div class="row mt-4">
        <?php
        // Fetch all notices with files
        $notices = $conn->query("SELECT title, attachment FROM notices ORDER BY notice_date DESC");

        // Check if notices exist
        if ($notices->num_rows > 0):
            while ($row = $notices->fetch_assoc()):
                $filePath = "noticepic/" . htmlspecialchars($row['attachment']); // File path
                $fileExtension = pathinfo($row['attachment'], PATHINFO_EXTENSION); // Get file type
        ?>
        <div class="col-md-4 mb-4">  <!-- Using col-md-4 for better layout -->
            <div class="card h-100 shadow-sm">
                <?php if (in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png', 'gif'])): ?>
                    <!-- Display image if it's an image file -->
                    <img src="<?= $filePath ?>" class="card-img-top img-fluid" alt="<?= htmlspecialchars($row['title']) ?>" style="object-fit: cover; height: 200px;">
                <?php elseif (strtolower($fileExtension) === 'pdf'): ?>
                    <!-- Display a PDF icon preview -->
                    <div class="text-center p-3">
                        <a href="<?= $filePath ?>" target="_blank">
                            <img src="assets/pdf-icon.png" alt="PDF Preview" width="80"> 
                        </a>
                    </div>
                <?php else: ?>
                    ...
                    <div class="text-center p-3">
                        <a href="<?= $filePath ?>" target="_blank">
                            <img src="assets/file-icon.png" alt="File Preview" width="80">
                        </a>
                    </div>
                <?php endif; ?>
                ...
                <div class="card-body text-center">
                    <h5 class="card-title">
                        <a href="<?= $filePath ?>" target="_blank" class="text-primary fw-bold text-decoration-none">
